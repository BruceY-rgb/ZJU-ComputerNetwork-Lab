<img src="lab7_report/cover.png" alt="cover" style="zoom:43%;" />
<div align="left">
  <font face="楷体" size = 5>
    ...
    <font face="楷体" size = 5>
      &nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp姓&nbsp&nbsp&nbsp&nbsp名：</font><font face="楷体" size = 5><u>&nbsp&nbsp&nbsp 杨思行&nbsp&nbsp&nbsp</u>
  </font><br/><br/>
    <font face="楷体" size = 5>
    ...
    <font face="楷体" size = 5>
      &nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp学&nbsp&nbsp&nbsp&nbsp号：<u>&nbsp&nbsp&nbsp3230104703&nbsp&nbsp&nbsp</u>
  </font><br/><br/>
    <font face="楷体" size = 5>
...
<div align="left">  
 ...
 &nbsp&nbsp&nbsp</u>
         ...
         &nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp学生姓名:<u>&nbsp&nbsp杨思行&nbsp</u>专业: <u>&nbsp&nbsp 计算机科学与技术&nbsp&nbsp</u>学号: <u>&nbsp&nbsp 3230104703&nbsp&nbsp</u>
        </font><br/><br/><font face="黑体" size=4>
         &nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp同组学生姓名:<u>&nbsp&nbsp&nbsp&nbsp祝子豪&nbsp&nbsp&nbsp</u>指导老师: <u>&nbsp&nbsp&nbsp&nbsp陆系群&nbsp&nbsp&nbsp&nbsp</u>
        </font><br/><br/><font face="黑体" size=4>
         &nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp实验地点:<u>&nbsp&nbsp&nbsp曹光彪西-304 &nbsp&nbsp&nbsp</u>实验日期: <u>&nbsp&nbsp 2025年12月20日&nbsp&nbsp</u>
    </font><br/></div>
...
- 客户端选择连接功能时，客户端和服务端显示内容截图。
  ...
  - 客户端
  ![alt text](995cdf7f929c20dc8e5e51db8a1c1fbc.png) 

  Wireshark抓取的数据包截图：

  ![alt text](c0ab394af85d255e2a9769dee0d2adaa.png)
...
- 客户端选择获取时间功能时，客户端和服务端显示内容截图。
  ...
  - 客户端：
  ![alt text](ad684df8df31d0985b672efbd985a451.png) 
  - Wireshark抓取的数据包截图（展开应用层数据包，标记请求、响应类型、返回的时间数据对应的位置）：
    - 请求数据包：![alt text](lab7_report/test-2-w1.png)
    - 响应数据包：![alt text](lab7_report/test-2-w2.png)
...
- 客户端选择获取名字功能时，客户端和服务端显示内容截图。
  ...
  - 客户端：
   ![alt text](122409bf9dde1c5fef885c8ad1129c0c.png)

  - Wireshark抓取的数据包截图（展开应用层数据包，标记请求、响应类型、返回的名字数据对应的位置）：
    - 请求数据包：![alt text](lab7_report/test-3-w1.png)
    - 响应数据包：![alt text](lab7_report/test-3-w2.png)
...
- 客户端选择获取客户端列表功能时，客户端和服务端显示内容截图。
  ...
  - 客户端：
  ![alt text](lab7_report/test-4-c.png)

  Wireshark抓取的数据包截图（展开应用层数据包，标记请求、响应类型、返回的客户端列表数据对应的位置）：

  - 请求数据包：![alt text](lab7_report/test-4-w1.png)
  - 响应数据包：![alt text](lab7_report/test-4-w2.png)
...
- 客户端选择发送消息功能时，客户端和服务端显示内容截图。
  ...
  ![alt text](lab7_report/test-5-c.png)
  ...
  Wireshark抓取的数据包截图（发送和接收分别标记）：
    ...
    - 请求数据包：![alt text](lab7_report/test-5-w1.png)
    - 响应数据包：![alt text](lab7_report/test-5-w2.png)
...
- 拔掉客户端的网线，然后退出客户端程序。
  ...
  ![alt text](lab7_report/test-6.png)

- 再次连上客户端的网线，重新运行客户端程序。
  
  ![alt text](lab7_report/test-7.png)
...
- 修改获取时间功能，改为用户选择1次，程序内自动发送100次请求。
  ...
  ![alt text](lab7_report/test-8-c.png)
  ![alt text](lab7_report/test-8-w.png)
...
- 多个客户端同时连接服务器，同时发送时间请求（程序内自动连续调用100次send），服务器和客户端的运行截图
  ...
  ![alt text](628a579341bbe720f45099aa50d860c2.png)  
...
- 假设在服务端调用listen和调用accept之间设了一个调试断点，暂停在此断点时，此时客户端调用connect后是否马上能连接成功？
  ...
  **原理解释：**
  1. `listen()`函数的作用是将socket设置为被动监听模式，并创建一个**连接等待队列**（本实验中队列长度为20）
...
-   连续快速send多次数据后，通过Wireshark抓包看到的发送的Tcp Segment次数是否和send的次数完全一致？
    ...
    **不完全一致**，实际的TCP Segment数量通常**少于**`send()`调用次数。
...
- 服务器在同一个端口接收多个客户端的数据，如何能区分数据包是属于哪个客户端的？
  ...
  **答：**
  ...
  服务器通过**不同的socket文件描述符**来区分不同客户端的数据。
  ...
  **Socket机制的处理：**
  ...
  1. **监听socket和连接socket**：
     - 服务器使用一个**监听socket**绑定在固定端口4703上，只负责接受连接请求
     - 每当`accept()`成功时，会返回一个**新的socket文件描述符**（server.cpp:180），这个新socket专门用于与该客户端通信
...
- 客户端断网后异常退出，服务器的TCP连接状态有什么变化吗？服务器该如何检测连接是否继续有效？
  ...
  **答：**
  ...
  客户端断网后异常退出（如直接拔网线、kill -9进程、断电等），服务器的TCP连接状态**不会立即改变**，会长时间保持在**ESTABLISHED状态**（可能10分钟以上甚至更久，直到TCP keepalive超时或应用层检测到）。
